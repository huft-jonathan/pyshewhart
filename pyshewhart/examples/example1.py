...
def main():
    ...
    kwargs = {
        'units': 'Volts',
        'check_western_elec_rules': False}
    ...
    pyshewhart.XbarSControlChart(
        measurements,
        suptitle="No Time Info Provided",
        **kwargs)
    ...
    pyshewhart.XbarSControlChart(
        elapsed_times,
        measurements,
        suptitle="Time Provided as Elapsed Times",
        **kwargs)

    # Plot #3 __________________________________________________________________
    timestamps = [time.time()]
    for i in range(1, len(measurements)):
        timestamps.append(timestamps[i - 1] + 60 * 60 * 8 * random.lognormvariate(mu=0, sigma=1.6))
    pyshewhart.XbarSControlChart(
        timestamps,
        measurements,
    ...
    pyshewhart.XbarSControlChart(
        datetimes,
        measurements,
        suptitle="Time Provided as Datetimes",
        **kwargs)
    ...
    pyshewhart.XbarSControlChart(
        str_dts,
        measurements,
        suptitle="Time Provided as Strings",
        **kwargs)
